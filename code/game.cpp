...
void game_face(int level, int step_num, int restep_num,int restep_num3);
void pause();
void lastlevel();
...
int gameloop(int level)
{
	...
	//☆
	int restep_num;//记录剩余撤销次数//可能需要改动的地方
	...
	//☆
	int isUndo3;//记录是否使用大招
	int restep_num3;//记录剩余大招次数
	int box_nx3, box_ny3;//记录三次推箱子后箱的坐标
	int last_x3, last_y3;//记录三次推箱子前角色的坐标
	int box_nx2, box_ny2;//记录上一次推箱子后箱的坐标
	int last_x2, last_y2;//记录上一次推箱子前角色的坐标
	int boxmove;//记录箱子是否连续移动三次
					   //游戏初始化
	...
	for (; is_run(); delay_fps(60)) {
		...
		case 0:
			//游戏界面
			game_face(level, step_num, restep_num, restep_num3);
			...
			case 0:
				if (option == 's') {//向下移动
					...
					if (map[x + 1][y] == ' ' || map[x + 1][y] == 'X') {
						map[x][y] = ' ';
						//将角色当前位置更新为空地（如果该位置应该为目的地我们将在后续代码进行处理）
						x++;//更新角色位置
						map[x][y] = '@';//将新的位置更新为角色
						step_num++;//移动步数加1
						boxmove = 0;
				...
				//大招
				if (isUndo3>0 && restep_num3 > 0&& step_num>3&&boxmove>=3) {
					...
					map[x][y] = '@';//将角色还原的新位置更新为@
									...
									//同理更新箱子的位置
					map[box_x][box_y] = ' ';
					map[box_nx3][box_ny3] = 'O';

					step_num = step_num-3;//还原之前所走的步数

					isUndo3 = 0;//将撤销判断变量设置为0（false）表示当前箱子位置为撤销后的位置，禁止再次撤销
					isUndo = 0;
					restep_num3--;//剩余可撤销次数减1
				}
				}
...
void game_face(int level, int step_num, int restep_num ,int restep_num3)
{
	...
	sprintf(str, "剩余大招次数：%d", restep_num3);
	outtextxy(780, 350, str);

	outtextxy(780, 400, "重玩本关(Q)");
	outtextxy(780, 450, "撤销(U)");
	outtextxy(780, 500, "大招(K)");
	outtextxy(780, 550, "暂停(ESC)");
